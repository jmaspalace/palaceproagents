...
<section class="container-fluid bloque-banner">
    ...
    <section class="content">
        <h1><?= $node->field_title_edit_profile['und'][0]['value']; ?></h1>
    </section>
</section>
<?php
...
if ($_SESSION['agency'])
{
    $url = $node->field_url_profile_agency['und'][0]['value'];
    if (cdi_custom_get_market_by_language() == 'CA' && $node->field_url_profile_agency_ca['und'][0]['value'] != '')
    {
        $url = $node->field_url_profile_agency_ca['und'][0]['value'];
    }
    else if (cdi_custom_get_market_by_language() == 'LATAM' && $node->field_url_profile_agency_latam['und'][0]['value'] != '')
    {
      $url = $node->field_url_profile_agency_latam['und'][0]['value'];
    }
}
else if($_SESSION['agent'])
{
  $url = $node->field_url_profile_agent['und'][0]['value'];
  if (cdi_custom_get_market_by_language() == 'CA' && $node->field_url_profile_agent_ca['und'][0]['value'] != '')
  {
    $url = $node->field_url_profile_agent_ca['und'][0]['value'];
  }
  else if (cdi_custom_get_market_by_language() == 'LATAM' && $node->field_url_profile_agent_latam['und'][0]['value'] != '')
  {
    $url = $node->field_url_profile_agent_latam['und'][0]['value'];
  }
}
$url=$url."?crypt=".base64_encode($_SESSION['dataEncrypt']);
?>
